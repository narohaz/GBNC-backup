...
;
	document.addEventListener('DOMContentLoaded', function () { // 获取所有团队卡片
const teamCards = document.querySelectorAll('.gbnc-team-card');

// 创建弹窗元素
function createModal() { // 创建弹窗容器
const modalContainer = document.createElement('div');
modalContainer.className = 'gbnc-team-modal-container';
modalContainer.style.display = 'none';

// 创建弹窗内容
const modalHTML = `
				<div class="gbnc-team-modal">
					<div class="gbnc-modal-content">
						<div class="gbnc-modal-close">&times;</div>
						<div class="gbnc-modal-header">
							<h2 class="gbnc-modal-name"></h2>
						</div>
						<div class="gbnc-modal-body">
							<div class="gbnc-modal-image">
								<img src="" alt="团队成员">
							</div>
							<div class="gbnc-modal-profile"></div>
						</div>
					</div>
				</div>
			`;
...
const modalClose = modalContainer.querySelector('.gbnc-modal-close');

// 为每个团队卡片添加点击事件
teamCards.forEach(function (card, index) {
card.addEventListener('click', function () { // 获取当前卡片的内容
const name = card.querySelector('.gbnc-person-name').innerHTML;
const profile = card.querySelector('.gbnc-person-profile').innerHTML;
const imgSrc = card.querySelector('.gbnc-card-image img').getAttribute('src');

// 填充弹窗内容
const modalName = modalContainer.querySelector('.gbnc-modal-name');
const modalProfile = modalContainer.querySelector('.gbnc-modal-profile');
const modalImg = modalContainer.querySelector('.gbnc-modal-image img');

modalName.innerHTML = name;
modalProfile.innerHTML = profile;
