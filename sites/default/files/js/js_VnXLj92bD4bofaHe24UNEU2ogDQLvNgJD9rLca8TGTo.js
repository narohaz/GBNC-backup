...
;
(function ($) {
    $(document).ready(function () { // 点击团队成员卡片打开模态框
        $('.team-member-card').click(function () {
            var memberId = $(this).data('member-id');
            var memberName = $(this).find('.member-name').text();
            var memberTitle = $(this).find('.member-title').text();
            var memberImage = $(this).find('.member-image img').clone();
            var memberDescription = "{{ nothing }}";
            // 获取成员详细介绍

            // 填充模态框内容
            $('#team-member-modal .modal-name').text(memberName);
            $('#team-member-modal .modal-title').text(memberTitle);
            $('#team-member-modal .modal-description').html(memberDescription);
            $('#team-member-modal .modal-image').html(memberImage);
        ...
        // 点击模态框外部关闭
        $(document).on('click', '.team-modal', function (e) {
            if ($(e.target).hasClass('team-modal')) {
                $('#team-member-modal').removeClass('active');
                $('body').css('overflow', 'auto');
            }
        });
        ...
        $(document).keydown(function (e) {
            if (e.keyCode === 27 && $('#team-member-modal').hasClass('active')) {
                $('#team-member-modal').removeClass('active');
                $('body').css('overflow', 'auto');
            }
        });
