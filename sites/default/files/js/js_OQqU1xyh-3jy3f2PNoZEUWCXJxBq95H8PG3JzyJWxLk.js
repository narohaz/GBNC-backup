...
(function ($) {
    ...
    $(document).ready(function () {
        // 导航滚动效果优化
        var $header = $('.gva-navigation-wrapper');
        var $navigation = $('.modern-navigation');
        var $navBg = $('.gva-navigation-bg');
        var lastScrollTop = 0;
        var navHeight = $header.outerHeight();
        var scrollThreshold = 50; // 进一步降低滚动阈值以更快显示固定导航
        var scrollTolerance = 30; // 为避免轻微滚动触发隐藏/显示

        // 强制设置导航背景元素高度与导航相同
        function updateNavBgHeight() {
            $navBg.css('height', $header.outerHeight() + 'px');
        }
        ...
        function handleNavigation() {
            ...
            if (currentScrollTop > scrollThreshold) {
                if (!$navigation.hasClass('nav-fixed')) {
                    ...
                    $navBg.css('transform', 'translateY(-100%)');
                    setTimeout(function () {
                ...
                if (currentScrollTop > lastScrollTop + scrollTolerance && currentScrollTop > navHeight + scrollThreshold) {
                    ...
                    $navBg.addClass('nav-hidden');
                }
                // 向上滚动超过容差值时显示
                else if (lastScrollTop > currentScrollTop + scrollTolerance) {
                    $navigation.removeClass('nav-hidden');
                    $navBg.removeClass('nav-hidden');
                }
            } else {
                $navigation.removeClass('nav-fixed nav-hidden');
                $navBg.removeClass('fixed nav-hidden');
        ...
        setTimeout(function () {
            ...
            $('.gva-navigation-bg').addClass('nav-loaded');
