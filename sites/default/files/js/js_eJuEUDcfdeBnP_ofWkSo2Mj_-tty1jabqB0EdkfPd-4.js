...
/**
 * 中文排版辅助处理函数
 * 用于在不支持现代CSS特性的浏览器中提供孤行孤字解决方案
 */
(function () {
    // 检测浏览器是否支持text-wrap: balance
    function supportsTextWrapBalance() {
        var testElem = document.createElement('div');
        return (
            'textWrap' in testElem.style ||
            'WebkitTextWrap' in testElem.style ||
            'MozTextWrap' in testElem.style
        );
    }
    ...
    if (!supportsTextWrapBalance()) {
        // 获取所有中文段落
        var paragraphs = document.querySelectorAll('.p1, .p2');
    ...
    function balanceText(element) {
        ...
        for (var i = 0; i < chars.length; i++) {
            ...
            if (i < chars.length - 1) {
                // 在汉字后或标点符号前添加零宽空格
                var isCJK = /[\u4e00-\u9fa5]/.test(chars[i]);
                var isNextCJK = /[\u4e00-\u9fa5]/.test(chars[i + 1]);
                var isPunctuation = /[，。、；：？！""''（）【】《》]/.test(chars[i + 1]);

                if ((isCJK && !isNextCJK) || (!isPunctuation && i % 15 === 0)) {
                    result += '\u200B'; // 零宽空格
                }
            }
        }
